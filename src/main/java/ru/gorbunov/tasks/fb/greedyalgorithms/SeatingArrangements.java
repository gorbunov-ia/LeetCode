 ...
 * permutations of seat assignments. Once the guests have sat down, the awkwardness between a pair of guests sitting
 * in adjacent seats is defined as the absolute difference between their two heights. Note that, because the table is
 * circular, seats 1 and n are considered to be adjacent to one another, and that there are therefore n pairs of
 * adjacent guests. The overall awkwardness of the seating arrangement is then defined as the maximum awkwardness of
 * any pair of adjacent guests. Determine the minimum possible overall awkwardness of any seating arrangement.
 * Signature
...
public class SeatingArrangements {
    ...
    int minOverallAwkwardness(int[] arr) {
        ...
        int leftValue = arr[0];
        int rightValue = arr[1];
        int maxDistance = distance(leftValue, rightValue);

        for(int i = 2; i < arr.length; i++) {
            int currentValue = arr[i];
            if (i % 2 == 0) {
                maxDistance = Math.max(maxDistance, distance(currentValue, leftValue));
                leftValue = currentValue;
            } else {
                maxDistance = Math.max(maxDistance, distance(currentValue, rightValue));
                rightValue = currentValue;
            }
        }

        return Math.max(maxDistance, distance(leftValue, rightValue));
    }
}

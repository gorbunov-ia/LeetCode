...
public class EncryptedWords {
    ...
    private void encrypt(String s, StringBuilder accumulator) {
        if (s.isEmpty()) {
            return;
        }
        int mid;
        ...
        accumulator.append(s.charAt(mid));
        encrypt(s.substring(0, mid), accumulator);
        encrypt(s.substring(mid + 1), accumulator);
    }

 ...
 * a 5-cent coin. Given a list of the available denominations, determine if it's possible to receive exact change for
 * an amount of money targetMoney. Both the denominations and target amount will be given in generic units of that
 * currency.
 * Signature
...
public class ChangeInAForeignCurrency {
    ...
    boolean canGetExactChange(int targetMoney, int[] denominations) {
        ...
        for (int coin : denominations) {
            if ((targetMoney - coin) == 0 || (targetMoney - coin) > 0 && canGetExactChange(targetMoney - coin, denominations)) {
                return true;
            }
        }
        return false;
